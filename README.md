...
1. **模块化结�?*: 代码按功能分类到不同文件夹中，便于维�?
2. **分离输出**: 转录文本保存为txt文件，总结内容保存为md文件
3. **多模型支�?*: 支持Whisper的多种模�?tiny, base, small, medium, large)
4. **多种模板**: 提供多种提示词模板，适应不同场景
5. **配置管理**: 支持保存API密钥和默认设�?
6. **图形界面**: 提供友好的GUI界面，方便用户操�?
7. **批处理功�?*: 支持批量处理多个音频文件
8. **音频提取**: 支持从视频文件中提取音频
...
# 批量处理
python scripts/batch_process.py --source_folder 音频文件夹路�?--output 输出文件夹路�?

# 音频提取
python scripts/extract_audio_from_video.py 源文件夹路径 目标文件夹路�?
```
